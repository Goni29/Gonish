...
const FAST_TRACK_SURCHARGE = 5;
const DISCOUNT_CONDITION_SURCHARGE = 5;
const REQUIRED_DISCOUNT_CONDITION_IDS = ["portfolio", "review"] as const;
...
const scheduleOptions: Option[] = [
  { id: "relaxed", label: "여유 있게 진행하고 싶어요", description: "기본 작업 기간 안에서 차분히 진행해도 괜찮아요.", price: 0, score: 0 },
  { id: "normal", label: "평균적인 일정으로 진행하고 싶어요", description: "보통 3~7일 안에 진행하고 싶어요.", price: 0, score: 1 },
  {
    id: "fast",
    label: "조금 더 빠르게 진행하고 싶어요",
    description: "기본 일정에서 하루 정도 더 앞당겨야 하는 경우예요.",
    price: FAST_TRACK_SURCHARGE,
    score: 2,
  },
];
...
const featureOptions: Option[] = [
  ...
  { id: "social-login", label: "소셜 로그인", description: "카카오, 구글, 네이버 등 외부 계정으로 로그인하는 기능이에요.", price: 8, score: 1 },
  { id: "role-permission", label: "사용자 역할 / 권한 분리", description: "일반회원, 파트너, 매니저처럼 사용자 유형별 접근 범위를 나누는 기능이에요.", price: 8, score: 2 },
  { id: "admin-dashboard", label: "관리자 대시보드", description: "관리자 첫 화면에서 전체 현황을 요약해서 보는 화면이에요. 요약 카드, 최근 항목, 간단한 수치 확인이 포함될 수 있어요.", price: 10, score: 2 },
  { id: "admin-module", label: "관리자용 업무 관리 모듈", description: "회원 관리, 주문 관리, 예약 관리, 문의 관리처럼 운영자가 업무 데이터를 처리하는 기능 1개예요.", price: 10, score: 2 },
  { id: "admin-permission", label: "관리자 권한 세분화", description: "운영자 계정마다 볼 수 있는 메뉴와 수정 가능한 범위를 다르게 설정하는 기능이에요.", price: 8, score: 1 },
  { id: "stats-report", label: "고급 통계 / 리포트", description: "기간별 통계, 필터, 차트, 다운로드처럼 운영 데이터를 더 자세히 분석하는 기능이에요.", price: 10, score: 2 },
  { id: "crud-board", label: "게시판 / 콘텐츠 관리", description: "공지사항, 블로그, 포트폴리오처럼 콘텐츠를 등록·수정·삭제하는 기능이에요.", price: 10, score: 3 },
  { id: "payment", label: "온라인 결제", description: "일반적인 1회성 결제를 사이트에서 받을 수 있어요.", price: 15, score: 3 },
  { id: "subscription", label: "정기 결제 / 구독", description: "매달 또는 일정 주기로 자동 결제가 필요한 경우예요.", price: 23, score: 4 },
  { id: "map", label: "기본 지도 표시", description: "매장이나 회사 위치처럼 고정된 장소를 사이트에 보여주는 기능이에요.", price: 5, score: 1 },
  { id: "notification-email", label: "자동 이메일 발송", description: "문의 접수, 신청 완료, 알림 메일처럼 자동으로 이메일을 보내는 기능이에요.", price: 5, score: 1 },
  { id: "notification-sms", label: "문자 / 알림 발송", description: "신청 완료, 상태 변경, 예약 안내 등을 문자나 알림으로 보내는 기능이에요.", price: 8, score: 1 },
  { id: "external-api", label: "기타 외부 API 연동 1종", description: "지도, 이메일, 문자 외에 다른 외부 서비스와 연동이 필요한 경우예요.", price: 10, score: 2 },
];
...
const discountOptions: Option[] = [
  {
    id: "portfolio",
    label: "완성작 포트폴리오 사용 가능해요",
    description: "현재 안내 금액은 완성 후 일부 화면을 Gonish 포트폴리오에 소개할 수 있는 조건을 전제로 해요.",
    price: 0,
    score: 0,
  },
  {
    id: "review",
    label: "작업 후 짧은 리뷰 제공 가능해요",
    description: "현재 안내 금액은 프로젝트 종료 후 짧은 후기 제공 조건을 전제로 해요.",
    price: 0,
    score: 0,
  },
];
...
const initialForm: EstimateForm = {
  brand: "",
  domainHosting: "",
  discounts: [...REQUIRED_DISCOUNT_CONDITION_IDS],
  features: [],
  goal: "",
  name: "",
  note: "",
...
function getMissingDiscountConditionIds(discounts: string[]) {
  return REQUIRED_DISCOUNT_CONDITION_IDS.filter((id) => !discounts.includes(id));
}

function getDiscountConditionAdjustment(discounts: string[]) {
  return getMissingDiscountConditionIds(discounts).length * DISCOUNT_CONDITION_SURCHARGE;
}

function getDiscountConditionLabel(discounts: string[]) {
  const missingIds = getMissingDiscountConditionIds(discounts);

  if (missingIds.length === 0) {
    return "포트폴리오 사용 + 리뷰 제공 기준";
  }

  if (missingIds.length === 1) {
    return missingIds[0] === "portfolio" ? "포트폴리오 사용 미적용" : "리뷰 제공 미적용";
  }
...
function buildRevisionPolicy(scheduleKey: string) {
  if (scheduleKey === "fast") {
    return "빠른 일정 기준으로 범위를 우선 고정한 뒤 기본 6회 수정 포함. 큰 방향 변경은 별도 견적으로 재안내합니다.";
  }

  return "기본 6회 수정 포함. 큰 방향 변경이나 추가 기능 요청은 별도 견적으로 재안내합니다.";
}
...
function getDiscountStep(form: EstimateForm) {
  const missingIds = getMissingDiscountConditionIds(form.discounts);

  if (missingIds.length === 0) {
    return "현재 보이는 금액은 포트폴리오 사용과 리뷰 제공을 전제로 한 할인가예요. 마지막 내용만 적어주시면 상담 준비가 거의 끝나요.";
  }

  if (missingIds.length === 1) {
    return missingIds[0] === "portfolio"
      ? "포트폴리오 사용 조건이 빠지면 일반 견적으로 다시 계산돼요. 가능하면 두 조건을 함께 유지해 주세요."
      : "리뷰 제공 조건이 빠지면 일반 견적으로 다시 계산돼요. 가능하면 두 조건을 함께 유지해 주세요.";
  }

  return "두 조건이 모두 빠지면 현재보다 높은 일반 견적으로 다시 계산돼요.";
}
...
export default function EstimateConversation() {
  ...
  const discountConditionLabel = useMemo(() => getDiscountConditionLabel(form.discounts), [form.discounts]);
  ...
  const priceEstimate = useMemo(() => {
    const discountConditionAdjustment = getDiscountConditionAdjustment(form.discounts);
    const missingDiscountConditionIds = getMissingDiscountConditionIds(form.discounts);
    const hasSelection =
    ...
    if (!hasSelection) {
      return {
        ...
        urgentSurcharge: 0,
        label: formatPriceRange(MIN_START_PRICE, DEFAULT_MAX_PRICE),
    ...
    const scheduleSurcharge = selectedSchedule?.price ?? 0;

    const discountedBase =
      subtotalBeforeUrgent +
      scheduleSurcharge +
      discountConditionAdjustment;

    const min = roundToFive(Math.max(0, discountedBase));
    const bufferBase = min < 30 ? 10 : min < 80 ? 15 : min < 150 ? 20 : 30;
    const featureBuffer = selectedFeatures.length >= 6 ? 20 : selectedFeatures.length >= 3 ? 10 : 0;
    const largeScopeBuffer =
      form.pageScope === "multi-add" ? 10 : form.pageScope === "dashboard" || form.pageScope === "list-detail" ? 5 : 0;
    ...
    return {
      ...
      min,
      max,
      urgentSurcharge: scheduleSurcharge,
      label: formatPriceRange(min, max),
      description:
        scheduleSurcharge > 0
          ? missingDiscountConditionIds.length === 0
            ? "빠른 일정과 할인가 조건이 반영된 기준가예요."
            : "빠른 일정과 일반 견적 기준이 반영된 금액이에요."
          : missingDiscountConditionIds.length === 0
            ? "포트폴리오 사용과 리뷰 제공 조건을 반영한 할인가예요."
            : missingDiscountConditionIds.length === 1
              ? `${getDiscountConditionLabel(form.discounts)} 상태라 일반 견적 기준으로 다시 계산한 금액이에요.`
              : "포트폴리오 사용과 리뷰 제공 조건이 빠져 일반 견적 기준으로 계산한 금액이에요.",
    };
  }, [
    form.discounts,
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const emailData: EstimateEmailData = {
        ...
        discounts: discountConditionLabel,
          ...
          >
            ...
            >
              ...
              <EstimateCompactSummaryPanel
                ...
                selectedDiscountLabel={discountConditionLabel}
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="relative pl-10 sm:pl-12">
              ...
              <div className="space-y-16">
                ...
                <QuestionSection
                  number="07"
                    question="현재 안내 금액 기준을 적용해도 괜찮을까요?"
                    helper="현재 보이는 낮은 금액은 포트폴리오 사용과 리뷰 제공 두 조건을 전제로 해요. 빠지는 항목은 일반 견적으로 다시 계산됩니다."
                  >
          ...
          <aside className="hidden xl:sticky xl:top-6 xl:block xl:self-start">
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs leading-5 text-ink/62">
                  현재 공개 금액은 포트폴리오 사용과 리뷰 제공 조건을 전제로 하며, 조건이 빠지면 일반 견적으로 다시 안내드려요.
                </p>
                <p className="text-xs leading-5 text-ink/62">
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2.5 text-sm leading-6 text-ink-muted">
        ...
        <SummaryLine label="금액 기준" value={selectedDiscountLabel} />
      </div>
